...
public class ObjectPool<T> : ObjectPool where T : Component
{
    ...
    private readonly List<ObjectPoolItemAbstract> allItems = new List<ObjectPoolItemAbstract>();

    // Constructor. Defines the prefab and initializes some components.
    public ObjectPool(T prefab, Transform holder, int startCount = 1)
    {
        this.prefab = prefab;
        this.holder = holder;
    ...
    private T InstantiateNewObject()
    {
        ...
        if (!obj.TryGetComponent<ObjectPoolItemAbstract>(out var objectPoolItem))
        {
            Debug.LogWarning($"{obj.name} does not have a component that implements ObjectPoolItemAbstract");
            return obj;
        }

        // If object has the ObjectPoolItemAbstract, set this ObjectPool as it's pool and store in list
    ...
    public override void Release()
    {
        ...
        {
            allItems.Remove(item as ObjectPoolItemAbstract);
