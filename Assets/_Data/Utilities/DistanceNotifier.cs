...
public class DistanceNotifier
{
    ...
    /// <summary>
    ///     This event is broadcast whenever the distance condition is met. If checkInside is true then event will be broadcast
    ///     if the current distance
    ///     is less than distance, otherwise it will broadcast if it is greater. It will also broadcast continuously while
    ///     enabled is true
    /// </summary>
    ...
    {
        if (dist <= sqrDistance) OnNotify?.Invoke();
    }
